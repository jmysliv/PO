...
public class Course {
    ...
    public boolean enrollStudent(final Student student) {
        String enrollStudentSql = "select * from student_course where student_id = ? and course_id = ?";
        Object[] args = {
            student.id(),
                this.id
        };
        ...
        try {
            ResultSet rs = QueryExecutor.read(enrollStudentSql, args);
        ...
        try {
            QueryExecutor.create(insertSql, insertArgs);
            return true;
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Student> studentList() {
    	String findStudentListSql ="select s.id, s.first_name, s.last_name, s.index_number from " +
                "student_course as sc join " +
                "student as s " +
                "on sc.student_id = s.id where " +
                "sc.course_id = ?";
        Object[] args = {
            this.id
        };
