...
public class Student {
    ...
    public Map<Course, Float> createReport() {
        ...
        String getAvgSql = "select avg(grade) as avg, course_id from grade group by course_id having student_id = ?";
        Object[] args = {
                this.id
        };
        try {
            ResultSet rs = QueryExecutor.read(getAvgSql, args);
            while(rs.next()){
                report.put(Course.findById(rs.getInt("course_id")).get(), rs.getFloat("avg"));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
